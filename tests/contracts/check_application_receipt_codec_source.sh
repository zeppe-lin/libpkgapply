...
test_source=$root/tests/protocol/application_receipt_codec_test.cpp
...
for meson_path in \
  ...
  tests/protocol/application_receipt_codec_test.cpp \
  tests/contracts/check_application_receipt_codec_source.sh
do
...
grep -F "'protocol/application_receipt_codec_test.cpp'" "$root/tests/meson.build" >/dev/null ||
