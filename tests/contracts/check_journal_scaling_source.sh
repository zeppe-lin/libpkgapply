...
grep -F 'constexpr std::size_t effect_count = 512;' "$test_source" >/dev/null ||
  fail 'large progressive journal witness is absent'
grep -F 'large_events.size(),' "$test_source" >/dev/null ||
  fail 'large journal witness does not advance event prefixes'
